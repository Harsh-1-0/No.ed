...
const RoleSelector: React.FC<TagSelectorProps> = ({ onComplete }) => {
  const [selectedTag, setSelectedTag] = useState<string | null>(null);
  const [showModal, setShowModal] = useState(false);
  const router = useRouter();

  const tags = [
    ['Machine Learning', 'Cloud Computing', 'Artificial Intelligence', 'Quantum Computing'],
  ];
    ...
    <div className="w-full flex flex-col min-h-screen bg-white">
      ...
      <div className="flex-grow bg-black w-full relative">
        {tags.map((row, rowIndex) => (
          <div key={rowIndex} className="grid grid-cols-2 gap-2 m-2 h-full">
            {row.map((tag) => (
              <button
                key={tag}
